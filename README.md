...
npm test                                          # 98 테스트 (81 unit + 10 integration + 7 e2e)
...
- **local-first auth 포팅**: Claude 기본 live runner 를 Claude Code CLI 세션으로 전환. Claude/Codex/Gemini CLI 호출 직전 long-lived API key 환경변수는 기본 차단, 명시 opt-in 시에만 SDK/API-key 경로 사용. Provider 공통 core 유틸(`cli-resolver`, `subprocess`, `json-extractor`) 분리. Codex read-only 실행 전후 git mutation guard 추가. 단위 81 + 통합 10 + E2E 7 = **98/98 PASS**.
