// Stdio JSON-RPC. Read one JSON request from stdin and write one JSON response.
// Methods:
//   - "ping"
    ...
    match req.method.as_str() {
        ...
        "session.upsert" => {
            ...
            match crate::session::open(root).and_then(|c| {
                ...
                Ok(()) => Response {
                    id,
                    result: Some(serde_json::json!({ "ok": true, "session": sid })),
                    error: None,
                },
                Err(e) => Response {
                    id,
                    result: None,
                    error: Some(format!("upsert: {e}")),
                },
            }
        }
        "handoff.record" => {
            ...
            match crate::session::open(root).and_then(|c| {
                ...
                Err(e) => Response {
                    id,
                    result: None,
                    error: Some(format!("handoff: {e}")),
                },
            }
        }
        "session.list" => {
            match crate::session::open(root).and_then(|c| crate::session::list_active(&c)) {
                Ok(rows) => Response {
                    id,
                    result: Some(serde_json::json!({ "active": rows })),
                    error: None,
                },
                Err(e) => Response {
                    id,
