...
NEKOWORK `0.1.0-alpha.8` is the current repository alpha candidate. The published `@ps-neko/nekowork@alpha` package currently points at `0.1.0-alpha.7` until the owner completes the alpha.8 publish. Use npm alpha for the shortest first-run path, or use a source checkout, submodule, or local repository integration when you need examples, tests, or repository-pinned workflows.
