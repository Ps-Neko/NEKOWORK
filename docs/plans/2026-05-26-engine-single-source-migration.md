...
**Goal:** monorepo(`C:/Users/Mun/harness`)에서 중복된 헤비 엔진 `packages/forge-engine` 과 그 타입-전용 패키지 `packages/quality-core` 를 제거하여, standalone `C:/Users/Mun/NEKOFORGE` 를 헤비 제품의 **단일 소스**로 확정한다. 라이트 제품 `packages/nekowork-cli` 는 영향받지 않는다.
...
Run: `cd /c/Users/Mun/harness && grep -rn "from ['\"].*\(forge-engine\|quality-core\)" packages/nekowork-cli/docs/visualizer/src`
...
Run: `cd /c/Users/Mun/NEKOFORGE && npm run verify`
...
Run: `cd /c/Users/Mun/harness && git checkout main && git pull --ff-only && git checkout -b chore/single-source-engine`
