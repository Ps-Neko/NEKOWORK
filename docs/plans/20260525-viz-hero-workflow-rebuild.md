...
**작업 디렉토리:** 모든 상대 경로는 `packages/nekowork-cli/docs/visualizer/` 기준. 명령은 monorepo 루트(`REPO_ROOT`)에서 `pnpm --filter @ps-neko/visualizer <script>` 로 실행.
...
Run:
```bash
git -C REPO_ROOT checkout -b feat/viz-hero-workflow-rebuild
```
...
```bash
git -C REPO_ROOT add packages/nekowork-cli/docs/visualizer/src/hero.ts packages/nekowork-cli/docs/visualizer/src/renderer.ts packages/nekowork-cli/docs/visualizer/src/main.ts packages/nekowork-cli/docs/visualizer/src/styles.css packages/nekowork-cli/docs/visualizer/tests/a11y.test.ts
git -C REPO_ROOT commit -m "feat(visualizer): 워크플로우 Before/After 토글 hero (wedge 대체)"
```
...
```bash
git -C REPO_ROOT add packages/nekowork-cli/docs/visualizer/src/renderer.ts packages/nekowork-cli/docs/visualizer/src/styles.css packages/nekowork-cli/docs/visualizer/tests/a11y.test.ts
git -C REPO_ROOT commit -m "refactor(visualizer): PR summary 제거 + conflict 헤더로 PR 식별 흡수"
```
...
```bash
git -C REPO_ROOT rm packages/nekowork-cli/docs/visualizer/src/wedge.ts
```
...
```bash
git -C REPO_ROOT add packages/nekowork-cli/docs/visualizer/src/styles.css
git -C REPO_ROOT commit -m "refactor(visualizer): 미사용 wedge.ts + .wedge 스타일 제거"
```
...
```bash
git -C REPO_ROOT add packages/nekowork-cli/docs/visualizer/tests/a11y.test.ts
git -C REPO_ROOT commit -m "test(visualizer): hero 토글 키보드 접근성 회귀 테스트"
```
...
```bash
git -C REPO_ROOT add packages/nekowork-cli/docs/visualizer/scripts/gen-hero-gif.ts packages/nekowork-cli/assets/hero.gif
git -C REPO_ROOT commit -m "feat(visualizer): hero GIF 를 before/after 토글 2프레임으로 재생성"
```
...
Run:
```bash
node REPO_ROOT/scripts/ci/check-markers.js
```
Expected: PASS.

- [ ] **Step 6: 최종 상태 확인**

Run: `git -C REPO_ROOT status --short && git -C REPO_ROOT log --oneline -8`
