...
| 7 | Parallel Processing | `team`; planned `auto --parallel-candidates N` | Partial | Read-only team perspectives exist. Isolated candidate writers are planned and must not share one target worktree. |
...
Current team mode is read-only. It gives multiple perspectives but does not yet make multiple patch candidates.

The planned `auto --parallel-candidates N` pattern should work like this:
