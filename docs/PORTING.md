...
NEKOWORK `0.1.0-alpha.7` is the current public alpha. The published `@ps-neko/nekowork@alpha` package points at `0.1.0-alpha.7`. Use npm alpha for the shortest published install path, or use a submodule/local checkout for repository-pinned workflows and examples.
