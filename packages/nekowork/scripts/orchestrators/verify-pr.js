...
export function printVerifyPrSummary(result) {
  ...
  if (decision.verdict === VERDICT.INSUFFICIENT_EVIDENCE) {
    ...
    if (decision.project?.checks_available?.test) {
      console.log('     a test command was detected, but this gate only detects checks — it does not run them.');
      console.log('     -> run your tests via CI or the @ps-neko/nekowork-harness runtime, or pass --ci-exit-soft to avoid blocking CI.');
    } else {
