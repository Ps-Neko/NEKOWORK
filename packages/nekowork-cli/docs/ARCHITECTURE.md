...
- Public npm alpha is the slim `@ps-neko/nekowork@alpha`, currently `0.2.0-alpha.8`, which now ships all **11 rules** and adds **one tiny, well-known dependency** (`acorn`, the JS parser — MIT, zero transitive dependencies) for the AST engine. The `latest` dist-tag is still a stale `0.2.0-alpha.0` (5 rules, **zero dependencies**), so always install with `@alpha`. The internal harness runtime `@ps-neko/nekowork-harness` is `private` and not published (source-checkout only).
