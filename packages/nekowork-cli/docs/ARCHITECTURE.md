...
- Public npm alpha is the slim `@ps-neko/nekowork@alpha`, currently `0.2.0-alpha.12`, which now ships all **11 rules** and adds **one tiny, well-known dependency** (`acorn`, the JS parser — MIT, zero transitive dependencies) for the AST engine. The `latest` dist-tag can lag behind `@alpha` (currently `0.2.0-alpha.11`), so always install with `@alpha`. The internal harness runtime `@ps-neko/nekowork-harness` is `private` and not published (source-checkout only).
