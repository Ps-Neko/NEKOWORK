...
- Public npm alpha is the slim `@ps-neko/nekowork@alpha`, currently `0.2.0-alpha.2`. The internal harness runtime `@ps-neko/nekowork-harness` is `private` and not published (source-checkout only).
