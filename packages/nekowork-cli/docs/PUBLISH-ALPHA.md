...
|---|---|---|---|
| `@ps-neko/nekowork` | Public **slim verification gate** (4 verbs: `check` / `verify-pr` / `report` / `apply`) | **published** (`@alpha` dist-tag) | `0.2.0-alpha.4` |
...
-> { latest: '0.2.0-alpha.0', alpha: '0.2.0-alpha.4' }
```
...
1. Bump `packages/nekowork/package.json` `version` to the next alpha (e.g. `0.2.0-alpha.4`).
...
> The published `@alpha` may lag behind `main`. Pin an exact version (`@ps-neko/nekowork@0.2.0-alpha.4`) for reproducible behavior.
