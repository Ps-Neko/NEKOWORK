...
NEKOWORK should stay narrow. It is not a domain-interview, DDD, or product-discovery tool. The published slim package (`@ps-neko/nekowork@alpha`) does one thing: read a diff, run deterministic risk rules, and emit a verdict. It begins at the trust boundary — verify the AI-made change and record evidence — and never commits, pushes, merges, or deploys.
...
`check`, `verify-pr`, `report`, `apply`. For CI and pre-commit you only need
`verify-pr`. Always pin the `@alpha` tag — bare/`latest` resolves to a stale
`0.2.0-alpha.0`.
...
> **Pre-1.0 alpha pin.** There is no stable release tag yet — published tags stop
> at `v0.1.0-alpha.12`, and the slim product line is `0.2.0-alpha.x`. Pin
> `@main` to a specific commit SHA, or to a released alpha tag once one matches
> your needs, rather than tracking a moving branch in production CI. The action's
> own `version:` input defaults to the `@alpha` dist-tag so it pulls the alpha
> line, not the stale `latest`.

Equivalent direct invocation (no composite action):
...
repos:
  - repo: https://github.com/Ps-Neko/NEKOWORK
    rev: main          # pre-1.0 alpha: pin to a commit SHA or a released tag
    hooks:
...
The hook entry runs `npx -y @ps-neko/nekowork@alpha verify-pr --from-staged`, so
it always pulls the alpha line.

Requires Python's `pre-commit` tool (`pip install pre-commit`).

> Tags stop at `v0.1.0-alpha.12` today, so do not pin `rev: v1` (it does not
> exist). Pin a commit SHA for reproducibility, or a released tag once the
> product reaches a stable release.

## Heavy harness (source checkout only)

The richer session-based runtime — `ask`, `plan`, `team`, `work`, `verify`,
`gate`, `ship`, `run`, `build`, `review`, and more — lives in the internal
`@ps-neko/nekowork-harness` package, which is **NOT published to npm**. It only
runs from a source checkout, and the slim package rejects those verbs. The
artifact contract below applies to that heavy runtime, not to the slim npm flow.

### Artifact Contract

These files are optional, but they are the recommended handoff contract between upstream discovery/planning workflows and the heavy harness.
...
Recommended flow (heavy harness):
...
If these files do not exist, the heavy harness still works from the task prompt and session artifacts. The contract is for richer upstream workflows, not a new requirement for every change.
