...
function buildAutoSection() {
  ...
  lines.push('## 핵심 명령어');
  lines.push('');
  lines.push('README 의 Main Surface 3계층(Beginner / Advanced / Legacy) 과 정렬한다. 신규 사용자는 Beginner 4종만 먼저 익히면 된다.');
  lines.push('');
  lines.push('### Beginner — 안전 게이트 4종');
  lines.push('');
  lines.push('```bash');
  lines.push('nekowork check                              # 환경 진단 (30초)');
  lines.push('nekowork start "<task>"                     # 검증 결과(verdict 카드) 우선 출력');
  lines.push('nekowork report --session latest            # 세션 증거 → readable REPORT.md');
  lines.push('nekowork apply --session <id>               # 명시적 적용. SHIP_READY 와 clear gate 필수');
  lines.push('```');
  lines.push('');
  lines.push('### Advanced — 단계별 제어');
  lines.push('');
  lines.push('```bash');
  lines.push('nekowork ask "<task>"                       # question gate, no project mutation');
  lines.push('nekowork plan "<task>"                      # plan-only');
  lines.push('nekowork team "<task>"                      # read-only worker handoffs');
  lines.push('nekowork work "<task>"                      # single executor implement handoff');
  lines.push('nekowork verify "<task>" --session <id>     # Codex-only verification');
  lines.push('nekowork gate status --session <id>         # HUMAN_GATE / approval / block 확인');
  lines.push('nekowork ship "<task>" --session <id>       # SHIP_READY / NO_SHIP 결정');
  lines.push('nekowork run "<task>" --session <id>        # work -> verify -> ship 래퍼');
  lines.push('nekowork build "<task>" [--mode auto|...]   # 일체형 빌더 래퍼');
  lines.push('nekowork auto "<task>" [--level cautious|...]  # bounded autonomy (apply 는 별도)');
  lines.push('nekowork pr-prep --session <id>             # PR 자료만, 브랜치/푸시/PR 생성 X');
  lines.push('```');
  lines.push('');
  lines.push('### Legacy — 하위 호환');
  lines.push('');
  lines.push('```bash');
  lines.push('nekowork review "<task>" [--secure|--fast|--no-ship]   # legacy full cycle');
  lines.push('nekowork review-cycle "<task>"                          # explicit legacy alias');
  lines.push('nekowork self-review');
  lines.push('nekowork codex-review                       # 단계 5 단독');
  lines.push('```');
  lines.push('');
  lines.push('### Install / Diagnostics');
  lines.push('');
  lines.push('```bash');
  lines.push('nekowork install --plan --profile core      # 설치 dry-run');
  lines.push('nekowork install --plan --pack quality      # curated pack dry-run');
  lines.push('nekowork install --apply --profile <p>      # 실제 설치');
  lines.push('nekowork sessions                           # 세션 목록');
  lines.push('nekowork costs --since=7d                   # 비용 추정');
  lines.push('```');
  lines.push('');
