...
> **Reproducibility note:** `npx @ps-neko/nekowork@alpha` resolves to the most recently published alpha. The published alpha may lag behind `main`. Pin an exact version (e.g. `@ps-neko/nekowork@0.2.0-alpha.10`) for reproducible behavior.
...
NEKOWORK is for teams that want AI-assisted development without making the agent catalog the product. The default path keeps local auth, inspectable handoffs, single-executor writes, independent Codex advisor notes (which never control the deterministic verdict), and Human Gate decisions in front of risky ship/apply steps.
...
Current repository version: `0.1.0-alpha.12` (the internal harness runtime `@ps-neko/nekowork-harness` — `private`, source-checkout only, not published) · Current npm alpha: `@ps-neko/nekowork@0.2.0-alpha.10` (the public slim verification gate, `@alpha` dist-tag). CLI: `nekowork` (the published slim package exposes the `nekowork` bin only). Default: mock providers, no API keys.
...
- **Demos & evidence:** [DEMO](docs/DEMO.md) · [DEMO-REPORT](docs/DEMO-REPORT.md) · [EXTERNAL-RUN](docs/EXTERNAL-RUN.md) · [legacy session case-studies](docs/case-studies)
